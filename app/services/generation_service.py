...
class MockLLMClient(LLMProvider):
    ...
    async def generate_structured_slides(
        ...
        middle_templates = [
            ("agenda", "共有項目", ["進捗の要点", "課題の整理", "次の打ち手"], "layout2"),
            ("content", "重点案件の停滞が課題", ["受注見込みが後ろ倒し", "優先順位が案件別でぶれる"], "layout2"),
            ("content", "優先案件へ工数を再配分する", ["重点案件へ営業工数を寄せる", "週次レビューを固定化する"], "layout3"),
            ("content", "進捗確認の精度を高める", ["案件更新の粒度をそろえる", "判断材料を早く集約する"], "layout2"),
        ]
